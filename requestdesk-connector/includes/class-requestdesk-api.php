...
<<<<<<< HEAD
        try {
            ...
            $this->log_pull_request($per_page, $offset, count($posts), $modified_since);
...
>>>>>>> master
     ...
     */
    private function log_pull_request($per_page, $offset, $returned_count, $modified_since = null) {
        // Simple logging to WordPress error log
        error_log(sprintf(
            'RequestDesk: Pull request - per_page: %d, offset: %d, returned: %d, modified_since: %s',
            $per_page,
            $offset,
            $returned_count,
            $modified_since ?? 'none'
